...
export default async function renderOrder(navItems, productCardsList) {
  ...
  const unavailableItems = updatedOrder
    .filter(el => !el.isAvailable)
    .map(el => el.name);

 // Рассчитываем сумму только для доступных товаров
  const sum = updatedOrder
    .filter(el => el.isAvailable)
    .reduce((acc, el) => acc + parseInt(el.price, 10), 0);
