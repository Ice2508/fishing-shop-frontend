  ...
  try {
    const strHtml = cards.map(card => `
      <li class="actions__cards-item">
        <span class="actions__cards-wrap actions__cards-wrap--price"><strong>${card.title}</strong></span>
        <div>
          <span>Цена:</span>
    ...
    priceInputs.forEach((input, i) => {
      ...
      // Настраиваем кнопку через универсальную функцию
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          // После успешного обновления обновляем оригинальное значение
